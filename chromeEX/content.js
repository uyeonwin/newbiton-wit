...
function findCard(el) {
  // 알바몬의 클래스명이 자주 바뀌므로 특정 클래스명보다
  // "크기 + 텍스트 + 근무지 주소 존재 여부"로 실제 공고 카드를 찾습니다.
  let node = el;
...
function extractCleanAddress(cardText) {
  ...
  const patterns = [
    // 서울 강남구 논현동 / 부산 해운대구 우동
    /(?:서울|부산|대구|인천|광주|대전|울산|세종)\s+[가-힣]+(?:구|군)\s+[가-힣0-9]+(?:동|읍|면|가|로|길)?(?:\s+\d+(?:-\d+)?)?/,

    // 경기 화성시 반월동 / 경기 고양시 덕양구 화정동
    /(?:경기|강원|충북|충남|전북|전남|경북|경남|제주)\s+[가-힣]+(?:시|군)\s+(?:[가-힣]+구\s+)?[가-힣0-9]+(?:동|읍|면|가|로|길)?(?:\s+\d+(?:-\d+)?)?/,

    // 서울특별시 성북구 안암로 145
    /[가-힣]+(?:특별시|광역시|특별자치시|특별자치도|도)\s+[가-힣]+(?:시|군|구)\s+(?:[가-힣]+구\s+)?[가-힣0-9]+(?:동|읍|면|가|로|길)\s*\d*(?:-\d+)?/
  ];
...
function collectCandidates() {
  ...
  for (const el of nodes) {
    ...
    // 부모와 자식이 모두 같은 급여를 잡는 경우 가장 안쪽 요소만 사용합니다.
...
function createBadge(el) {
  // 알바몬(React)이 자식 DOM을 다시 렌더링하며 제거하는 경우가 있어
  // 별도 span을 삽입하지 않고 급여 요소의 data-* 속성 + ::after로 표시합니다.
...
function openDetail({
  ...
  const startLabel =
    user.startPos === "home" ? "집" : "학교";

  const endLabel =
    user.endPos === "home" ? "집" : "학교";
...
async function calculateCandidate(candidate, user) {
  ...
  try {
    ...
    const commuteHours =
      (Number(toRoute.time) + Number(fromRoute.time)) / 60;

    const totalCost =
      Number(toRoute.cost) + Number(fromRoute.cost);

    const denominator =
      Number(workHours) + commuteHours;
...
async function injectWageBadges() {
  ...
  try {
    ...
    // 중복 실행을 막기 위해 API 요청 전에 카드 상태를 먼저 표시
    batch.forEach(({ card }) => {
...
// 알바몬은 화면 일부를 React로 다시 그릴 수 있으므로,
// 공고 DOM이 바뀌면 새 요소를 다시 스캔합니다.
